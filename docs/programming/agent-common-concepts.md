...
下面这张图可以先作为整体关系的入口。它可以按从左到右读：左侧的任务意图和长期背景先汇入 Context，Agent 基于 Context 做判断，再进入观察、行动和验证。Harness 是外层运行环境，Tools、MCP、Hook、Permissions 决定它能怎么行动、怎么被约束。

```mermaid
flowchart LR
    subgraph inputs[输入与长期背景]
        direction TB
        prompt[Prompt<br/>当前任务意图]
        rules[Rules<br/>长期约束]
        memory[Memory<br/>长期背景]
        skill[Skill<br/>特定场景流程]
    end

    context[Context<br/>当前可见信息]
    agent[Agent<br/>理解任务 / 决策 / 推进]

    subgraph workloop[行动循环]
        direction TB
        observe[观察<br/>读文件 / 看日志 / 查结果]
        act[行动<br/>改文件 / 跑命令 / 调 API]
        verify[验证<br/>测试 / lint / review]
    end
...
可以先从左侧看起。Prompt 是这次任务的意图，比如“修这个 bug”或者“写一段 SQL”。Rules 是长期约束，比如不要改某些目录、确认和授权时用中文。Memory 是过去沉淀下来的背景，Skill 是某类任务会用到的流程。它们会一起汇入 Context，形成 Agent 当前能看到的信息环境。

Context 右侧的 Agent 是决策点。它不是单纯生成一句回答，而是在当前 Context 里判断下一步该做什么。真正让它从“会聊天”变成“能干活”的，是图里的行动循环：

- 需要更多信息时，Agent 进入观察：读文件、看日志、查数据、看测试结果。
- 观察结果回到 Context，Agent 再判断下一步。
- 需要改变环境时，Agent 进入行动：改文件、跑命令、调用工具或 API。
- 行动后进入验证：跑测试、lint、review，确认结果是不是对的。
- 验证结果也回到 Context，再影响 Agent 下一轮判断。

这里不一定是每一轮都严格走完“观察、行动、验证”三个节点。更准确地说，Agent 每一轮都会基于 Context 做一次判断：信息不够就先观察，需要改环境就行动，行动之后再验证。观察结果和验证结果都会回到 Context，下一轮判断就建立在新的信息上。

比如让 coding agent 修一个测试失败的问题，它一般不会只说“可能是某个函数有问题”。它会先看失败日志，再搜相关代码；这些观察结果进入 Context 之后，Agent 才决定要改哪里。改完文件后它再跑测试，如果测试还失败，新的错误信息又会回到 Context，进入下一轮判断。这就是图里 `Context -> Agent -> 观察 / 行动 / 验证 -> Context` 的循环。

旁边的 Harness 可以理解成 Agent 的工作环境。Tools 和 MCP 决定行动能调用哪些能力，本地文件、命令行、数据库、GitHub、浏览器，都属于这一层。Permissions 和 Sandbox 决定它能不能做某些动作，比如能不能写文件、能不能访问外部系统、危险操作要不要先等人确认。Hooks 则是在固定时机做检查，比如执行命令前拦一下，修改文件后跑格式化，结束前跑测试。

所以这张图想表达的重点是：Agent 的能力不只来自模型本身，而来自“模型 + 上下文 + 工具 + 权限 + 验证”这一整套系统。Prompt 给目标，Context 给信息，Agent 做判断，Tools 和 MCP 支撑行动，Hooks 和 Permissions 控制风险，观察和验证结果再回到 Context，任务就这样一轮一轮往前推进。

Plugin 和 Subagent 是这套系统上的扩展。Plugin 更像一个能力包，可以把 Skill、MCP、Hook 打包在一起。Subagent 则是任务拆分，把某个边界清楚的子任务交给专门的执行单元，最后把结果再带回 Context。

## Agent（分享过）

Agent 可以先理解成“会自己推进任务的助手”。

普通聊天更像一次问答：用户问一句，模型答一句。Agent 则会多一个循环：先基于当前 Context 判断下一步，如果信息不够就去观察环境，如果需要改变结果就调用工具行动，拿到新的观察或验证结果后继续判断，直到任务完成。

比如让一个 coding agent 修一个测试失败的问题，它不会只回答“可能是某某文件有问题”。它会先看测试输出，再搜代码、读文件，把这些信息放回上下文；等它判断出该改哪里之后，再修改代码、重新跑测试。这个过程里，模型一直在做判断，但真正读取环境、改变环境和验证结果的是工具。
...
以招聘中的Job Description举例，岗位要求的技能就是Skill、岗位就是Plugin。在设置Plugin时，也要注意边界，大而全不一定好，尽量分工明确。
...
第一层，先写 Rules。把长期不变的项目习惯和安全边界写清楚，越短越好。**BI业务知识**
